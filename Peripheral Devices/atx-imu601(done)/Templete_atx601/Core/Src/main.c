...
int main(void)
{
  ...
  {
		/* ªÒ»°Õ”¬›“« ˝æ› */
    atk_ms601m_get_attitude(&attitude_dat, 100);                            /* ◊ÀÃ¨Ω« */
    atk_ms601m_get_gyro_accelerometer(&gyro_dat, &accelerometer_dat, 100);  /* Õ”¬›“«∫Õº”ÀŸ∂» */
...
/* USER CODE BEGIN 4 */
//ÂæÆÁßíÁ∫ßÂª∂Êó∂ÔºåÁî®‰∫éÈô?Ëû∫‰ª™‰º†Ëæì
void delay_us_hal(uint16_t nus)
{
    __HAL_TIM_SET_COUNTER(&htim6, 0); // ËÆ°Êï∞Âô®Ê∏ÖÈõ?
    HAL_TIM_Base_Start(&htim6);       // ÂêØÂä®ÂÆöÊó∂Âô?
    while (__HAL_TIM_GET_COUNTER(&htim6) < nus); // Á≠âÂæÖËÆ°Êï∞ËææÂà∞nus
    HAL_TIM_Base_Stop(&htim6);        // ÂÅúÊ≠¢ÂÆöÊó∂Âô?
}
/* USER CODE END 4 */
