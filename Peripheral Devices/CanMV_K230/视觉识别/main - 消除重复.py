...
def digit_recognition_thread():
    ...
    confidence_threshold = deploy_conf.get("confidence_threshold", 0.6) # 表示一个预测结果最小可信度阈值，用于过滤低概率结果,修改为0.6
    ...
    while not det_stop:
        ...
        try:
            ...
            # 2. 手动执行NMS
            final_detections = []
            manual_nms_threshold = 0.4 # 可以独立于库的NMS阈值，按需调整
            ...
            while len(all_detections) > 0:
                ...
                # 筛选掉剩下框中与 best_detection 重叠度过高的框
            ...
            # 接下来，使用过滤后的 final_detections 来进行后续处理
            ...
            for det in final_detections:
                ...
                if name.isdigit():
                    ...
                    # 注意：box的格式可能是[x1, y1, w, h]或[x1, y1, x2, y2]，需要确认
                    # 假设是[x1, y1, w, h]或[x1, y1, x2, y2]，x_coordinate都是box[0]
            ...
            det_app.draw_result(det_osd_img, filtered_res) # 只显示过滤后的结果
