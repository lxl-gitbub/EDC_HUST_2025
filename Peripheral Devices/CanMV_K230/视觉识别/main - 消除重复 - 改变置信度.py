...
def digit_recognition_thread():
    ...
    confidence_threshold = deploy_conf.get("confidence_threshold", 0.6) # 置信度阈值已设为0.6
    ...
    while not det_stop:
        ...
        try:
            ...
            # ---- 核心逻辑开始 ----
            # 1. 将所有置信度 > 0.6 的检测结果组合成一个列表
            all_detections = []
            for i in range(len(idxs)):
                if scores[i] >= confidence_threshold: # 只有置信度达标的才会被处理
            ...
            # 3. 手动执行跨类别NMS，消除重叠框
            final_detections = []
            manual_nms_threshold = 0.4 # 可按需调整
            ...
            # ---- NMS处理结束，final_detections 是最终干净的结果 ----

            # 4. 基于最终结果(final_detections)准备当前帧的数据
            ...
            # 5. 确定要发送的数据（如果当前帧有识别，用当前的；否则用上一次的）
            ...
            # 7. 在屏幕上绘制最终结果 (final_detections)
            # 因为 final_detections 已经经过了置信度筛选和NMS，所以这里绘制的就是最终输出的数字
            ...
            det_osd_img.clear() # 清除上一帧的绘制内容
            det_app.draw_result(det_osd_img, filtered_res_for_display) # 只显示过滤后的结果
