...
/**
 * @brief       获取并显示ATK-MS601M数据 (将原 demo_key0_fun 改为通用函数)
 * @param       无
 * @retval      无
 */
static void get_and_display_ms601m_data(void)
{
...
{
    uint8_t ret;
    uint8_t key; /* 保持 key 变量以便如果将来需要按键功能 */
    ...
    // 初始化时先发送一次数据，确保启动后立即有输出
    get_and_display_ms601m_data(); 

    while (1)
    {
        // 移除按键扫描的switch-case结构，直接在主循环中实现定时发送
        // key = key_scan(0); // 这一行可以保留，但不再用于控制数据发送
